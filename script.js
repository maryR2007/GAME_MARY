...
const heroWidth = 21; // 24
const heroHeight = 50; // 40
  ...
  case "falling": {
   ...
   heroY += (timestamp - lastTimestamp) / fallingSpeed;
    ...
    if (heroY > maxHeroY) {
      ...
      document.getElementById("gameOver").style.display = "block"; // mostrar texto, sonido
      return;
    }
    break;
  }
...
function drawHero() {
  ...
  const legDistance = 7;
  ctx.beginPath();
  ctx.arc(legDistance, 12, 3, 0, Math.PI * 2, false);
  ctx.fill();
  ctx.beginPath();
  ctx.arc(-legDistance, 12, 3, 0, Math.PI * 2, false);
  ctx.fill();
...
function drawBackground() {
  ...
  gradient.addColorStop(0, "#fae903");
  gradient.addColorStop(1, "#fc9904");
  ctx.fillStyle = gradient;
  ctx.fillRect(0, 0, window.innerWidth, window.innerHeight);

  // Draw hills
  drawHill(hill1BaseHeight, hill1Amplitude, hill1Stretch, "#aefd04");
  drawHill(hill2BaseHeight, hill2Amplitude, hill2Stretch, "#203802");
